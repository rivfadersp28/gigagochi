...
def test_full_story_plans_events_before_rendering(monkeypatch) -> None:
    ...
    assert "дорожные метки, вешки, указатели" in completions.calls[0]["messages"][0]["content"]
    assert "дорожных меток, вешек, указателей" in completions.calls[1]["messages"][0]["content"]
    assert "дорожных меток, вешек, указателей" in completions.calls[3]["messages"][0]["content"]
