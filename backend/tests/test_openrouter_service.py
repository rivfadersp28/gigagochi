...
def test_openrouter_models_and_headers() -> None:
    ...
    assert get_character_model(settings) == "~openai/gpt-latest"
...
def test_character_model_can_be_overridden() -> None:
    settings = openrouter_settings(openrouter_character_model="openai/gpt-5-mini")
    assert get_character_model(settings) == "openai/gpt-5-mini"

    openai_settings = openrouter_settings(
        ai_provider="openai",
