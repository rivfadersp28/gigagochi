...
SYSTEM_PROMPT = "\n".join(
    (
        ...
        "текущего ответа.",
        "В части 1 поле openingContext одним коротким предложением говорит, где персонаж и зачем "
        "он здесь. Каждый элемент storyParagraphs — ровно одно короткое предложение с одним "
        "новым фактом. Вместе они создают первую ситуацию и останавливаются перед решением. "
        "У первой ситуации ещё нет исхода или изменений параметров: они появляются только после "
        "ответа владельца.",
...
def start_interactive_travel(
    *,
    ...
    user_content = (
        ...
        "В introReaction дай одну короткую законченную реплику персонажа о выбранном месте: "
        "удивление, азарт, любопытство, лёгкую тревогу или восторг. Не упоминай интерфейс, "
        "пользователя и генерацию истории. "
        "В openingContext одним простым предложением скажи, где персонаж и зачем он здесь. "
        "В storyParagraphs дай 2–3 коротких предложения: что случилось и какое возникло "
        "препятствие. Каждая строка сообщает ровно один факт и останавливается до поступка и его "
        "результата. Первая часть должна закончиться одним простым прямым challenge-вопросом "
