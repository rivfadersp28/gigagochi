...
def story_direction_block(
    ...
    *,
    ...
    antagonist_class = direction.get("antagonistClass", "")
    wonder_class = direction.get("wonderClass", "")
    ...
    if compact:
        compact_plot_instruction = {
            "encounter": (
                "герой встречает участника с простой практической целью; одно различие "
                "интересов развивается через понятные поступки и завершается в этой истории"
            ),
            "social_event": (
                "конкретная просьба, разногласие или совместное дело меняет отношения; "
                "цели сторон понятны из обычного разговора и действий"
            ),
            "mystery": (
                "сначала неизвестная причина наблюдаемой проблемы раскрывается через два "
                "обычных факта; причиной оказывается конкретное действие участника, поведение "
                "животного или влияние погоды, а ответ требует значимого выбора о безопасности "
                "или отношениях"
            ),
            "pursuit_or_conflict": (
                "участник активно преследует или мешает герою ради ясно показанной цели; "
                "решение следует из выбора героя и поведения этого участника"
            ),
            "rescue_or_help": (
                "участник или место оказывается в понятной трудности; герой помогает одним "
                "простым действием, после которого сразу виден результат"
            ),
            "discovery": (
                "герой замечает понятный факт в уже показанной ситуации; новое знание "
                "помогает выбрать обычное разумное действие и меняет его возможности"
            ),
            "exploration": (
                "герой входит в необычное место с устойчивым явлением, существами или укладом; "
                "каждая часть раскрывает новую сторону самого места и меняет отношение героя к нему"
            ),
            "environmental_event": (
                "видимое природное изменение заставляет героя выбрать безопасное и понятное "
                "действие; последствия этого выбора сразу наблюдаемы"
            ),
            "peaceful_change": (
                "обычное тёплое, смешное или красивое изменение получает развитие и заметно "
                "меняет настроение, отношения или ближайшую цель героя"
            ),
        }.get(plot_mode, STORY_DIRECTION_SPECS[plot_mode]["instruction"])
        compact_setting_instruction = {
            "inhabited_place": "простое обитаемое место с ясным назначением",
            "ancient_site": "старое заброшенное место, прежнее назначение которого понятно",
            "liminal_place": "граница двух обычных пространств, например леса и дороги",
            "remote_landscape": "горы, пустошь или другая дальняя природная местность",
            "underground": "пещера или подземное помещение с понятной планировкой",
        }.get(setting_class, STORY_SETTING_INSTRUCTIONS[setting_class])
        if plot_mode == "pursuit_or_conflict" and arc_variant == "theft":
            compact_plot_instruction = (
                "кража или насильственная потеря запускает активный поиск и столкновение "
                "с виновником, у которого есть собственная цель"
            )
        if plot_mode == "pursuit_or_conflict" and arc_variant == "battle":
            compact_plot_instruction = (
                "одна серьёзная битва занимает все четыре части; противник активно сражается "
                "в каждой части, бой усиливается, получает перелом и завершается решающей схваткой"
            )
        if plot_mode == "pursuit_or_conflict" and arc_variant == "chase":
            compact_plot_instruction = (
                "активный преследователь давит на героя во всех четырёх частях; погоня "
                "усиливается, меняет направление и получает окончательный исход"
            )
        compact_pursuit_arc = (
            "1) кража или потеря; 2) активный поиск либо погоня; 3) столкновение с "
            "виновником; 4) возвращение, окончательная потеря или осознанная цена"
            if arc_variant == "theft"
            else "1) первое столкновение; 2) усиление битвы и способности противника; "
            "3) перелом с ценой для героя; 4) решающая схватка и её последствия"
            if arc_variant == "battle"
            else "1) нападение, преследование или открытый вызов; 2) давление противника "
            "усиливается; 3) герой меняет баланс сил; 4) противостояние получает "
            "полный исход"
        )
        compact_arc_instruction = {
            "encounter": (
                "1) встреча и цели сторон; 2) взаимодействие меняет первое впечатление; "
                "3) выбор между союзом, спором или расхождением; 4) новое отношение или статус"
            ),
            "exploration": (
                "1) вход в необычное место и его главное чудо; 2) более глубокое знакомство "
                "с явлением, существами или укладом; 3) выбор, как с ними взаимодействовать; "
                "4) новое знание, связь с местом или изменившаяся цель"
            ),
            "mystery": (
                "1) наблюдаемая загадка; 2) две связанные улики; 3) конкретный ответ; "
                "4) последствие раскрытия"
            ),
            "social_event": (
                "1) просьба, знакомство или разногласие; 2) проявляются разные интересы; "
                "3) решение или признание; 4) отношения заметно меняются"
            ),
            "pursuit_or_conflict": compact_pursuit_arc,
            "rescue_or_help": (
                "1) показана чужая беда; 2) первая помощь осложняется; 3) герой находит "
                "работающий способ; 4) положение спасаемого действительно меняется"
            ),
            "discovery": (
                "1) герой сталкивается с необычным местом, существом или явлением; "
                "2) узнаёт его глубже; 3) понимает важное свойство или смысл; "
                "4) открытие создаёт новую возможность или меняет героя"
            ),
            "environmental_event": (
                "1) начинается крупное изменение среды; 2) оно затрагивает важное место или "
                "участников; 3) герой принимает трудное решение; 4) виден новый порядок "
                "после события"
            ),
            "peaceful_change": (
                "1) появляется приятная возможность, гость или событие; 2) герой включается "
                "в него; "
                "3) возникает значимый общий момент; 4) остаётся радость, связь или новая традиция"
            ),
        }[plot_mode]
        antagonist_instructions = {
        ...
        extra_lines = ""
        if antagonist_class in antagonist_instructions:
            extra_lines += (
                f"- Обязательный класс противника: "
                f"{antagonist_instructions[antagonist_class]}.\n"
            )
        if wonder_class in wonder_instructions:
            extra_lines += (
                f"- Обязательный тип необычного места: "
                f"{wonder_instructions[wonder_class]}.\n"
            )
        return (
            "STORY_DIRECTION: широкая рамка истории, а не список обязательных элементов. "
            "Собери естественный сюжет и самостоятельно выбери подходящих участников, "
            "препятствие и способ разрешения.\n"
            f"- Тип развития: {compact_plot_instruction}.\n"
            f"- Динамика четырёх частей: {compact_arc_instruction}.\n"
            f"{extra_lines}"
            f"- Исходное происшествие: {STORY_INCIDENT_INSTRUCTIONS[incident_class]}.\n"
            f"- Среда: {compact_setting_instruction}.\n"
            f"- Общий итог: {valence_instruction}.\n"
            "Не пытайся отдельно иллюстрировать каждую строку рамки: одна ясная причинная "
            "линия важнее полного набора мотивов."
        )
    return (
