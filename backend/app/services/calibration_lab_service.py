...
def _lore_seed_for_variant(prompt_variant: str, seed: str) -> dict[str, str]:
    ...
    if prompt_variant == "tiny_story_cards":
        lore_seed.update(
            {
                "body_mechanism": "заметный признак меняется от маленького действия",
                "behavior_trigger": "при заботе признак становится спокойнее или ярче",
                "habitat_pressure": "домом служит простое место, где тело чувствует себя безопасно",
                "growth_clue": "рост показывает более точный контроль главного признака",
            }
        )
    elif prompt_variant == "game_dialogue_cards":
        lore_seed.update(
            {
                "body_mechanism": "главная часть тела дает короткие игровые реакции",
                "behavior_trigger": "при вопросе creature показывает состояние через тело",
                "habitat_pressure": "среда дает один конкретный ресурс для восстановления",
                "growth_clue": "каждая стадия добавляет более уверенную реакцию",
            }
        )
    elif prompt_variant == "mixed_cards":
        lore_seed.update(
            {
                "body_mechanism": "одна физическая особенность хранит запас элемента",
                "behavior_trigger": "стресс проявляется как изменение цвета, звука или формы",
                "habitat_pressure": "опасность связана с потерей запаса энергии",
                "growth_clue": "future reveal касается скрытого предела способности",
            }
        )
    return lore_seed
